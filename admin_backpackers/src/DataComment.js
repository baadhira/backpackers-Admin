...
import { getReportedComments, getReportedEvents, getReportedUsers, getUsers } from "../src/methods/MethodUsers";
...
<table>
  ...
  <tbody>
    {data?.map(data=>
    <tr>
      <td >{data?.comment.from_discussion}</td>
      <td >{data?.reporter}</td>
      <td >{data?.reason}</td>
      {/* <td >{data?.comment}</td> */}
      <td><Button variant="primary " className="p-3"  style={{backgroundColor:"white",width:"100px",height:"40px",borderColor:"dodgerblue",borderRadius:"10px"}}>View Details</Button></td>
